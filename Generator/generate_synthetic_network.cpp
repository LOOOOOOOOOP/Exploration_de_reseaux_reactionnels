...
const size_t generate_number_of_electrons(const multiset<Atom>& atoms, const int& charge)
{
...
void add_atoms_to_ID(string& ID, const multiset<Atom>& atoms)
{
    // Ajout des atomes ordonnés selon la notation de Hill
    Atom C("C",6,0.25);
    Atom H("H",1,0.6);

    // S'il y a un carbone, on ajoute les C, puis les H
    ...
    // Ajout du reste des atomes en ordre alphabétique
...
void add_compound_number_to_ID(const Parameters& param, string& ID, const multiset<Atom>& atoms)
{
...
void add_conformer_number_to_ID(const Parameters& param, string& ID, const multiset<Atom>& atoms)
{
    static mt19937 generator(param.seed);
...
const string generate_system_ID(const Parameters& param, const multiset<Atom>& atoms, const size_t& n_electrons, const int& charge)
{
    string ID = "";
    /* Pas utilisé (Début de l'identifiant InChI)
    ID += "InChI=1S/";
    */
    ...
    ID = ID + param.compound_ID_connectivity_sublayer(atoms);

    // Ajout du sublayer des hydrogènes
    ID = ID + param.compound_ID_hydrogen_sublayer(atoms);
    */
    ...
    // Spécification du composé contenant le système (remplace sublayers de connectivité et des hydrogènes)
...
const string generate_system_ID(const Parameters& param, const multiset<Atom>& atoms, const size_t& n_electrons, const int& charge, const string& compound_number)
{
    string ID = "";
...
System generate_system(const Parameters& param)
{
    multiset<Atom> atoms = param.generate_atoms();
    int charge = param.generate_charge(atoms);
...
deque<System> generate_initial_systems(const Parameters& param)
{
    ...
    for (size_t i = 0; i < param.number_of_initial_systems; i++)
    {
        if (param.initial_systems_are_from_different_compounds == true)
        {
            ...
            do
            {
                ...
                {
                    if (calculate_compound_ID(initial_systems[j]) == calculate_compound_ID(S))
                    {
...
const float generate_barrier_between_compound_neighbours(const Parameters& param, const System& R, const System& P)
{
...
const float generate_barrier_between_class_neighbours(const Parameters& param, const System& R, const System& P)
{
...
const float generate_hyperedge_barrier(const Parameters& param, const multiset<System>& R, const multiset<System>& P)
{
...
void generate_compound_neighbour(const Parameters& param, Network& N, Class& C, const System& R)
{
...
void generate_compound_neighbourhood(const Parameters& param, Network& N, const System& S)
{
    string S_class_ID = calculate_class_ID(S);
    string S_compound_ID = calculate_compound_ID(S);
    string S_system_ID = S.system_ID;
    Class& C = N.classes.find(S_class_ID)->second;
    Compound& D = C.class_compounds.find(S_compound_ID)->second;
...
void generate_class_neighbour(const Parameters& param, Network& N, Class& C, const System& R)
{
...
void generate_class_neighbourhood(const Parameters& param, Network& N, const System& S)
{
    string S_class_ID = calculate_class_ID(S);
    string S_compound_ID = calculate_compound_ID(S);
    string S_system_ID = S.system_ID;
...
void choose_a_pair_of_systems_for_hyperedge(const Parameters& param, const Network& N, multiset<System>& R)
{
    static mt19937 generator(param.seed);
    ...
    const Class& C_1 = next(N.classes.begin(),class_index_1)->second;
    ...
    const Compound& D_1 = next(C_1.class_compounds.begin(),compound_index_1)->second;
    ...
    const System& S_1 = next(D_1.compound_systems.begin(),system_index_1)->second;

    R.insert(S_1);

    // Deuxième réactif
    int class_index_2 = random_class_index(generator);
    const Class& C_2 = next(N.classes.begin(),class_index_2)->second;
    ...
    const Compound& D_2 = next(C_2.class_compounds.begin(),compound_index_2)->second;
    ...
    const System& S_2 = next(D_2.compound_systems.begin(),system_index_2)->second;
...
const System& choose_system_for_splitting_hyperedge(const Parameters& param, const Network& N)
{
    static mt19937 generator(param.seed);
    ...
    do
    {
        ...
        const Class& C = next(N.classes.begin(),class_index)->second;
        ...
        const Compound& D = next(C.class_compounds.begin(),compound_index)->second;
...
void generate_hyperedge_from_pair(const Parameters& param, Network& N, const multiset<System>& R)
{
...
void generate_splitting_hyperedge(const Parameters& param, Network& N, const System& S)
{
...
const Network generate_synthetic_network(const Parameters& param)
{
    deque<System> initial_systems = generate_initial_systems(param);
    const Network N = generate_synthetic_network(param,initial_systems);
    return N;
}
/////////////////////////////////////////////////////////////////////

const Network generate_synthetic_network(const Parameters& param, deque<System>& initial_systems)
{
