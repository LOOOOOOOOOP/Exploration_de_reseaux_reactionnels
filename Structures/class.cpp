...
void Class::add_system_in_class_from_edge(size_t& number_of_systems, vector<System>& compound_unexplored_systems, vector<System>& class_unexplored_systems, const System& R, System& P, const float& RP_barrier, const float& PR_barrier)
{
    bool new_product_system = true; // Indique si le système P existe déjà dans le réseau
    System* already_existing_P;     // Si P existe déjà dans le réseau, pointeur vers P

    // Ajout du système dans le bon composé
    const string ID = calculate_compound_ID(P);
    ...
    if (number_of_systems_in_class == 1 && new_product_system == true)    // Si la classe vient d'être créée
    {
        vector<float> v = {-1};
        edges.push_back(v);
    ...
    else    // La classe existait déjà
    {
        if (new_product_system == true) // Si un nouveau système a été ajouté
        {
            ...
            {
                if (j == R.insertion_rank_in_class)
                {
